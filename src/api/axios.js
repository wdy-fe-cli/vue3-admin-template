...
import { version } from '@/config/request'
  ...
  constructor(temConfig) {
    ...
    this.service.interceptors.request.use(
      (config) => {
        ...
        if (config.headers && typeof config.headers.set === 'function' && token) {
    ...
    this.service.interceptors.response.use(
      (response) => {
        ...
        if (response.status === ResultEnum.OVERDUE) {
          goLogin()
          let message = data.subMessage ? data.subMessage : data.message
      ...
      async (error) => {
        ...
        if (response) {
          ...
          if (response.data.subCode === 'TOKEN_MODULE_FAIL') {
            let message = response.data.subMessage
